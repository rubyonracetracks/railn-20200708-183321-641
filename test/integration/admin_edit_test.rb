...
class AdminEditTest < ActionDispatch::IntegrationTest
  ...
  def edit_all_but_email(a, uname, name, password_n, password_c)
    edit_admin_start(a)
    ...
    page.assert_selector(:xpath, xpath_input_str(name))
    click_on 'Logout'
  ...
  def edit_all(a, uname, e, name, password_n, password_c)
    edit_admin_start(a)
    fill_in('Username', with: uname)
    ...
    assert page.has_text?('Please check your email and follow the confirmation link')
